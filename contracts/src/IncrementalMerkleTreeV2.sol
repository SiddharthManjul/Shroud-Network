...
/**
 * @title IncrementalMerkleTreeV2
 * @notice Append-only Poseidon Merkle tree for the Unified Shielded Pool.
 *
 * Properties:
 *   - Depth      : 24  ->  capacity: 2^24 = 16,777,216 leaves
 ...
 *
 * Larger than IncrementalMerkleTree (depth 20) to accommodate all tokens
 * sharing a single tree in the unified multi-token pool.
 *
 * @dev Zero values for levels 0-20 are identical to IncrementalMerkleTree.
 *      Levels 21-24 computed from the same Poseidon parameters via circomlibjs.
 */
library IncrementalMerkleTreeV2 {
    uint256 internal constant DEPTH = 24;
    uint256 internal constant MAX_LEAVES = 2 ** DEPTH; // 16,777,216
    ...
    // Levels 0-20: identical to IncrementalMerkleTree (V1)
    // Levels 21-23: extend the sequence for depth 24
    //
    // Generated from circomlibjs with canonical Poseidon parameters.
    // -----------------------------------------------------------------------
    function _zeros(uint256 i) internal pure returns (uint256) {
        ...
        // Levels 21-24: Poseidon(zeros[i-1], zeros[i-1])
        // Computed via circomlibjs with canonical Poseidon parameters.
        if (i == 21)
            return
        ...
        revert("IncrementalMerkleTreeV2: depth out of range");
    }
    ...
    function init(TreeData storage self, address poseidon) internal {
        require(poseidon != address(0), "IMT2: zero poseidon address");
    ...
    function insert(
        ...
        require(self.nextIndex < MAX_LEAVES, "IMT2: tree full");
